...
void FDialogueEdGraphConnector::VisitStartNode(UDialogueEdGraphStartNode* StartNode)
{
    ...
    const UDialogueGraphNode* NextRuntimeNode = *EditorToRuntime->Find(StartNode->GetNextNode());
...
void FDialogueEdGraphConnector::VisitSelectNode(UDialogueEdGraphSelectNode* SelectNode)
{
    ...
    Algo::Transform(SelectNode->GetNextNodes(), NextNodeGuids, [this](const UDialogueEdGraphNode* EditorNode)
    {
        ...
        if (CurrentNode)
        {
            return (*EditorToRuntime->Find(CurrentNode))->GetNodeID();
        }
        return InvalidGuid;
    });
         ...
         const UDialogueEdGraphNode* NextNode : NextNodes)
    {
        ...
        SelectionItem->SetNextNodeToVisit(*EditorToRuntime->Find(Node));
...
{
    UDialogueSceneNode* RuntimeLineNode = Cast<UDialogueSceneNode>(*EditorToRuntime->Find(LineNode));
